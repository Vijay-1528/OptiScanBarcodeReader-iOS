...
public class CameraFeedManager: NSObject {
    ...
    private let triggerSoundID = 1016
    ...
    private func setupBlinkLabel(_ installedDate: Date) {
        ...
        let waterMarkLabelItem = UILabel(frame: CGRect(x: maskContainer.origin.x - 5, y: maskContainer.origin.y + 300, width: 150, height: 50))
        waterMarkLabelItem.textColor = .lightGray
        waterMarkLabelItem.font = UIFont.systemFont(ofSize: 15, weight: .semibold)
        waterMarkLabelItem.text = "SCANFLOW"
    ...
    private func addVideoDeviceInput() -> Bool {
        ...
        do {
            ...
            camera.ramp(toVideoZoomFactor: 3, withRate: 1)
        ...
        if inference.className == "QR" {
            ...
            if !imgSize {
                ...
                if result.first != nil && result.first != "" {
                    ...
                    YoloV4Classifier.shared.isProduction() ? nil : AudioServicesPlaySystemSound(SystemSoundID(triggerSoundID))
                ...
                DebugPrint(message: "Before image save", function: .processResults)
                YoloV4Classifier.shared.isProduction() ? nil : UIImageWriteToSavedPhotosAlbum(originalBufferImage!, self, #selector(image(_:didFinishSavingWithError:contextInfo:)), nil)
